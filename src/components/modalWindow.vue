      ...
      <button class="position-absolute btn-close" @click="closeModal">
        ...
        <form @submit.prevent="submitCode" action="" class="form">
          <div class="position-relative">
            <input :class="{ 'input-error': isValidName }" v-model="name" class="" id="i1" placeholder="Ваше имя">
            <label for="i1">Имя</label>
            <span v-if="isValidName"  class="error">Некорректное имя</span>
          </div>
          <div class="position-relative">
            <vue-tel-input :class="{ 'input-error': isValidPhone }" id="id2" styleClasses="my-input" @validate="validPhone($event.valid)" v-model="phone"></vue-tel-input>
            <label for="i2">Телефон</label>
            <span v-if="isValidPhone" class="error">Некорректный номер телефона</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    validPhone(lolo){
      console.log(lolo)
      if(lolo===false) this.isValidPhone = true
